...
local function analyzeBee(stack)--返回值只有部分基因能正确表示显隐性关系
    ...
    for _,t in pairs(tag.Genome.value.Chromosomes.value) do
        if not t.value or not t.value.Slot or not t.value.Slot.value then
            local serialization = require("serialization")
            local bot = require("bot")
            local file = io.open("ErrorReport.txt", "w")
            file:write("param.stack =\n"..serialization.serialize(stack, 2000).."\n")
            file:write("tag =：\n"..serialization.serialize(tag, 1000).."\n")
            file:write("bot.inventory =\n"..serialization.serialize(bot.inventory, 5000).."\n")
            file:close()
            error("蜜蜂NBT数据格式错误，错误报告已生成。此前曾发生过该错误但一直无法复现，如果您遇到了此错误，请前往官方新手群联系开发者\"火山盐奶茶\"，并提供生成的错误报告(ErrorReport.txt)以协助修复此错误，感谢您的帮助。")
        end
